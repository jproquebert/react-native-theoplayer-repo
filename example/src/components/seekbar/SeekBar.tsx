...
export class SeekBar extends PureComponent<SeekBarProps, SeekBarState> {
  ...
  private get duration(): number {
    const { seekable } = this.props;
    return seekable.length > 0 ? seekable[seekable.length - 1].end - seekable[0].start : 0;
  }
  ...
  render() {
    ...
    const { focused } = this.state;
    const { isLive, style, progressDotStyle, labelStyle } = this.props;
    ...
    return (
      <View style={[styles.container, style]}>
        ...
        <TimeLabel style={[styles.timeLabel, labelStyle]} isLive={isLive} currentTime={currentTime} duration={this.duration} />
      </View>
    );
  }
}
